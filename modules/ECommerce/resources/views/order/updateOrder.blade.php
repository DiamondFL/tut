        ...
        <table class="table-responsive table">
            <form method="post" action="{{route('cart.change')}}">
                ...
                @for($i=0;$i<count($listId);$i++)
                    <?php $totalQty += $listQty[$i]; $totalMoney += \ECommerce\Models\Products::find($listId[$i])->price * $listQty[$i]?>
                    <?php $name = \ECommerce\Models\Products::find($listId[$i])->name; $price = \ECommerce\Models\Products::find($listId[$i])->price; ?>
                    <tr>
                        <td><input name="{{$listQty[$i]}}" type="number" value="{{$listQty[$i]}}" maxlength="2"
                                   size="3"></td>
                        <td>{{$name}}</td>
