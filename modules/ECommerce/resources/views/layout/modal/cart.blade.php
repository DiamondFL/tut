            ...
            <div class="modal-body table-responsive">
                @if(Session::has('cart'))
                    <table class="table-responsive table">
                        <form id="modalCart" method="post" action="{{route("cart.change")}}">
                            {{csrf_field()}}
                            ...
                            @foreach(Session::get('cart') as $key=>$value)
                                <?php $totalQty += $value; $totalMoney += \ECommerce\Models\Products::find($key)->price * $value?>
                                <?php $name = \ECommerce\Models\Products::find($key)->name; $price = \ECommerce\Models\Products::find($key)->price; ?>
                                <tr>
                                    <td><input name="{{$key}}" type="number" value="{{$value}}" maxlength="2" size="3">
                                    </td>
                                    <td>{{$name}}</td>
