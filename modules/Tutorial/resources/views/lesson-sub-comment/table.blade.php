<table class="table">
    <tr>
        <th><input type="checkbox" class="check_all"></th><th>{{trans('label.lesson_comment')}}</th>
...
<th>{{trans('label.is_active')}}</th>
    ...
    <tr>
        <td><input type="checkbox" class="check_item" data="{{$row->id}}"></td><td>{{$row->lesson_comment}}</td>
<td>{{$row->content}}</td>
<td>{{$row->create_by}}</td>
<td>{{$row->is_active}}</td>
